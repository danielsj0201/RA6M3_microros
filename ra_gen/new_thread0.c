...
static StaticTask_t new_thread0_memory;
                ...
                static uint8_t new_thread0_stack[16000] BSP_PLACE_IN_SECTION(BSP_UNINIT_SECTION_PREFIX ".stack.thread") BSP_ALIGN_VARIABLE(BSP_STACK_ALIGNMENT);
                #else
static uint8_t new_thread0_stack[16000] BSP_PLACE_IN_SECTION(BSP_UNINIT_SECTION_PREFIX ".stack.new_thread0") BSP_ALIGN_VARIABLE(BSP_STACK_ALIGNMENT);
#endif
#endif
...
void new_thread0_create(void)
{
    ...
    new_thread0 = xTaskCreateStatic (
                    ...
                    #endif
                                     new_thread0_func,
                                     (const char*) "New Thread", 16000 / 4, // In words, not bytes
